...
class ESIMMacher(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, hidden_dim, dropout_p=0.1, embedding_weight=None):
        super(ESIM, self).__init__()
        self.emb = nn.Embedding(num_embeddings, embedding_dim)
        if embedding_weight:
            self.emb.weight = nn.Parameter(embedding_weight)
        self.input_encoder = nn.LSTM(input_size=embedding_dim, hidden_size=embedding_dim, num_layers=2, batch_first=True, dropout=dropout_p, bidirectional=True)
    ...
    def forward(self, inputs):
        ...
        # Sentence embedding
        op, _ = self.input_encoder(xp) # (batch, sentence_len, embedding_dim)
        oh, _ = self.input_encoder(xh)
